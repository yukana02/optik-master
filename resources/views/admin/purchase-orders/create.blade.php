...
@push('styles')
<style>
.item-row { background:#f8f9fc; border-radius:8px; padding:12px 14px; margin-bottom:10px; border:1px solid #e9ecef; }
</style>
@endpush
@section('content')
        ...
        <div class="card h-100">
            ...
            <div class="card-body p-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tanggal_po" class="form-control" value="{{ old('tanggal_po', now()->format('Y-m-d')) }}" required>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Catatan</label>
                    <textarea name="catatan" class="form-control" rows="3" placeholder="Opsional...">{{ old('catatan') }}</textarea>
                </div>
                {{-- Total ringkasan --}}
    ...
    <div class="col-12 col-lg-8">
        <div class="card">
            <div class="card-header p-3 d-flex justify-content-between align-items-center">
                <span><i class="bi bi-box-seam text-primary me-2"></i>Item Produk</span>
            ...
            <div class="card-footer p-3 d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="bi bi-save me-1"></i>Simpan PO</button>
...
function addRow() {
    ...
    const idx = rowIdx++;
    const opts = products.map(p => `<option value="${p.id}" data-harga="${p.harga_beli}">${p.kode} — ${p.nama}</option>`).join('');
    const html = `
    <div class="item-row" id="row-${idx}">
        <div class="row g-2 align-items-end">
            <div class="col-12 col-md-5">
                <label class="form-label small fw-semibold mb-1">Produk</label>
                <select name="items[${idx}][product_id]" class="form-select form-select-sm" onchange="fillHarga(this,${idx})" required>
                    <option value="">— Pilih —</option>${opts}
                </select>
            </div>
            <div class="col-12 col-md-3">
                <label class="form-label small fw-semibold mb-1">Harga Beli (Rp)</label>
                <input type="number" name="items[${idx}][harga_beli]" id="harga-${idx}" class="form-control form-control-sm" min="0" oninput="recalc()" required>
            </div>
            <div class="col-6 col-md-2">
                <label class="form-label small fw-semibold mb-1">Jumlah</label>
                <input type="number" name="items[${idx}][qty]" id="qty-${idx}" class="form-control form-control-sm" value="1" min="1" oninput="recalc()" required>
            </div>
            <div class="col-6 col-md-1">
                <label class="form-label small fw-semibold mb-1">Subtotal</label>
                <div class="text-muted small fw-semibold pt-1" id="sub-${idx}">Rp 0</div>
            </div>
            <div class="col-6 col-md-1 text-end">
                <button type="button" class="btn btn-sm btn-light text-danger" onclick="removeRow(${idx})"><i class="bi bi-trash"></i></button>
            </div>
        </div>
    </div>`;
